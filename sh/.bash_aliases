...
Darwin)
    ...
    function proxy_sys() {
        ...
        local proxy_domain=$(netstat -nr | awk '/^0.0.0.0/ {print $2; exit} /^default/ {print $2; exit}')
...
alias ......='cd ../../../../..'
# short for du
alias du_1='du -h -d 1 | sort -h'
alias du_2='du -h -d 2 | sort -h'
alias du_3='du -h -d 3 | sort -h'
alias du_4='du -h -d 4 | sort -h'
...
# switch proxy on-off
# $1: proxy url; null to display; off to disable
function proxy_env() {
    if [ ${1:-NOCONFIG} = "NOCONFIG" ]; then
        if [ -n "${PROXY-}" ]; then
